...
def pelagic_conservation_report(g):
    ...
    upwelling_result = """r.mapcalc "upwresult = if(rresult==1,upwelling,null())" """
    ...
    #exposure stats
    chlorophyll_result = """r.mapcalc "chlresult = if(rresult==1,chlorophyll,null())" """
    ...
    for upw_id in upw_ids:
        ...
        upwelling_result = """r.mapcalc "%s = if(upwelling==%s,upwresult,null())" """ % (upw_raster, upw_id)
        g.run(upwelling_result)
        #generate chlorophyll results for current upwelling type
        upw_chl_raster = 'upwelling_%s_chlresult' %upw_id
        chl_result = """r.mapcalc "%s = if(%s==%s,chlorophyll,null())" """ %(upw_chl_raster, upw_raster, upw_id)
        g.run(chl_result)
        #generate dictionary from stats
